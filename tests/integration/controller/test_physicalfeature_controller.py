...
def test_should_return_single_physical_feature(
    ...
    response = client.get(
        f"/climsoft/v1/physical-features/{get_physical_feature.associatedWith}/{get_physical_feature.beginDate}/{get_physical_feature.classifiedInto}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_physical_feature(
    ...
    response = client.put(
        f"/climsoft/v1/physical-features/{associated_with}/{begin_date}/{classified_into}",
...
def test_should_fail_for_different_description(
    ...
    with caplog.at_level(logging.ERROR):
        client.get(
            f"/climsoft/v1/physical-features/{pf_input_data['associatedWith']}/{pf_input_data['beginDate']}/{pf_input_data['classifiedInto']}",
        )
