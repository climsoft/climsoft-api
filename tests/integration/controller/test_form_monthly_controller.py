...
def test_should_return_first_five_form_monthlys(client: TestClient, get_form_monthlys):
    response = client.get(
        "/test/climsoft/v1/form_monthlys",
        params={"limit": 5},
    )
...
def test_should_return_single_form_monthly(client: TestClient, get_form_monthly: climsoft_models.FormMonthly):
    response = client.get(
        f"/test/climsoft/v1/form_monthlys/{get_form_monthly.stationId}/{get_form_monthly.elementId}/{get_form_monthly.yyyy}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_form_monthly(client: TestClient, get_form_monthly):
    ...
    response = client.put(
        f"/test/climsoft/v1/form_monthlys/{station_id}/{element_id}/{yyyy}",
