...
def get_valid_station_element_input(
    station_id: str, instrument_id: str, element_id: int, schedule_class: str
):
    return stationelement_schema.StationElement(
        ...
        endDate=(datetime.datetime.utcnow() + datetime.timedelta(days=1234)).strftime(
            "%Y-%m-%dT%H:%M:%SZ"
        ),
    )
