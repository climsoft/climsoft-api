...
def test_should_return_single_physical_feature(
    ...
    response = client.get(
        f"/v1/physical-features/{get_physical_feature.associatedWith}/{get_physical_feature.beginDate}/{get_physical_feature.classifiedInto}/{get_physical_feature.description}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_physical_feature(
    ...
    response = client.put(
        f"/v1/physical-features/{associated_with}/{begin_date}/{classified_into}/{description}",
