...
def test_should_return_first_five_climsoft_users(
    ...
    response = client.get("/test/climsoft/v1/climsoft-users", params={"limit": 5})
...
def test_should_return_single_climsoft_user(
    ...
    response = client.get(
        f"/test/climsoft/v1/climsoft-users/{get_climsoft_user.userName}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_create_a_climsoft_user(client: TestClient):
    ...
    response = client.post(
        "/test/climsoft/v1/climsoft-users",
...
def test_should_raise_validation_error(client: TestClient):
    ...
    response = client.post(
        "/test/climsoft/v1/climsoft-users", data=json.dumps(climsoft_user_data, default=str)
    )
...
def test_should_update_climsoft_user(client: TestClient, get_climsoft_user):
    ...
    response = client.put(
        f"/test/climsoft/v1/climsoft-users/{get_climsoft_user.userName}/update-role/{target_role}"
    )
    response_data = response.json()
...
def test_should_delete_climsoft_user(client: TestClient, get_climsoft_user):
    response = client.delete(f"/test/climsoft/v1/climsoft-users/{get_climsoft_user.userName}")
    assert response.status_code == 200

    response = client.get(f"/test/climsoft/v1/climsoft-users/{get_climsoft_user.userName}")
