...
def test_should_return_single_form_synoptic_2_ra1(client: TestClient, get_form_synoptic_2_ra1: climsoft_models.FormSynoptic2Ra1):
    response = client.get(
        f"/v1/form_synoptic_2_ra1s/{get_form_synoptic_2_ra1.stationId}/{get_form_synoptic_2_ra1.yyyy}/{get_form_synoptic_2_ra1.mm}/{get_form_synoptic_2_ra1.dd}/{get_form_synoptic_2_ra1.hh}",
    )
    assert response.status_code == 200
    response_data = response.json()
