...
def get_qc_type_by_id(abbreviation: str, db_session: Session = Depends(deps.get_session)):
    try:
...
def create_qc_type(data: synopfeature_schema.CreateSynopFeature, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def update_qc_type(abbreviation: str, data: synopfeature_schema.UpdateSynopFeature, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
