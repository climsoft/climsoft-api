...
def get_instrument_by_id(instrument_id: str, db_session: Session = Depends(deps.get_session)):
    try:
...
def create_instrument(data: instrument_schema.CreateInstrument, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def update_instrument(instrument_id: str, data: instrument_schema.UpdateInstrument, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def delete_instrument(instrument_id: str, db_session: Session = Depends(deps.get_session)):
    try:
