...
def test_should_return_single_paper_archive(
    ...
    response = client.get(
        f"/climsoft/v1/paper-archives/{get_paper_archive.belongsTo}/{get_paper_archive.formDatetime}/{get_paper_archive.classifiedInto}",
        headers={"Authorization": f"Bearer {get_access_token}"},
    )
    print(response.json())
    assert response.status_code == 200
...
def test_should_update_paper_archive(
    ...
    paper_archive_data = climsoft_paper_archive.get_valid_paper_archive_input(
        station_id=get_paper_archive.belongsTo,
        paper_archive_definition_id=get_paper_archive.classifiedInto,
    ).dict(by_alias=True)
...
def test_should_delete_paper_archive(
    ...
    paper_archive_data = paperarchive_schema.PaperArchive.from_orm(
        get_paper_archive
    ).dict(by_alias=True)
