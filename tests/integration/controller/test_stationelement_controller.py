...
def test_should_return_single_station_element(
    ...
    response = client.get(
        f"/climsoft/v1/station-elements/{get_station_element.recordedFrom}/{get_station_element.describedBy}/{get_station_element.recordedWith}/{get_station_element.beginDate}",
        headers={"Authorization": f"Bearer {get_access_token}"},
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_delete_station_element(
    ...
    station_element_data = stationelement_schema.StationElement.from_orm(
        get_station_element
    ).dict(by_alias=True)
    recorded_from = station_element_data.pop("recorded_from")
    described_by = station_element_data.pop("described_by")
