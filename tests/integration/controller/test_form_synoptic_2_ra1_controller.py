...
def test_should_return_first_five_form_synoptic_2_ra1s(client: TestClient, get_form_synoptic_2_ra1s):
    response = client.get(
        "/test/climsoft/v1/form_synoptic_2_ra1s",
        params={"limit": 5},
    )
...
def test_should_return_single_form_synoptic_2_ra1(client: TestClient, get_form_synoptic_2_ra1: climsoft_models.FormSynoptic2Ra1):
    response = client.get(
        f"/test/climsoft/v1/form_synoptic_2_ra1s/{get_form_synoptic_2_ra1.stationId}/{get_form_synoptic_2_ra1.yyyy}/{get_form_synoptic_2_ra1.mm}/{get_form_synoptic_2_ra1.dd}/{get_form_synoptic_2_ra1.hh}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_form_synoptic_2_ra1(client: TestClient, get_form_synoptic_2_ra1):
    ...
    response = client.put(
        f"/test/climsoft/v1/form_synoptic_2_ra1s/{station_id}/{yyyy}/{mm}/{dd}/{hh}",
