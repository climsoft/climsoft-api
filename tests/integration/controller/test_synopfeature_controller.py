...
def test_should_return_single_synop_feature(
    ...
    response = client.get(
        f"/climsoft/v1/synop-features/{get_synop_feature.abbreviation}",
        headers={"Authorization": f"Bearer {get_access_token}"},
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_synop_feature(
    client: TestClient, get_synop_feature, get_access_token: str
):
    ...
    response = client.put(
        f"/climsoft/v1/synop-features/{abbreviation}",
...
def test_should_delete_synop_feature(
    client: TestClient, get_synop_feature, get_access_token: str
):
    synop_feature_data = synopfeature_schema.SynopFeature.from_orm(
        get_synop_feature
