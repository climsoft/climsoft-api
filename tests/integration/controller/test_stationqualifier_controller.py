...
def test_should_return_single_station_qualifier(
    ...
    response = client.get(
        f"/climsoft/v1/station-qualifiers/{get_station_qualifier.qualifier}/{get_station_qualifier.qualifierBeginDate}/{get_station_qualifier.qualifierEndDate}/{get_station_qualifier.belongsTo}",
        headers={"Authorization": f"Bearer {get_access_token}"},
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_station_qualifier(
    ...
    response = client.put(
        f"/climsoft/v1/station-qualifiers/{qualifier}/{qualifier_begin_date}/{qualifier_end_date}/{belongs_to}",
