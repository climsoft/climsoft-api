...
def get_data_form_by_id(form_name: str, db_session: Session = Depends(deps.get_session)):
    try:
...
def create_data_form(data: data_form_schema.CreateDataForm, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def update_data_form(form_name: str, data: data_form_schema.UpdateDataForm, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
