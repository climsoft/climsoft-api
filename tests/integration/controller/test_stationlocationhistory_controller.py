...
def test_should_return_single_station_location_history(
    ...
    response = client.get(
        f"/climsoft/v1/station-location-histories/{get_station_location_history.belongsTo}/{get_station_location_history.openingDatetime}",
        headers={"Authorization": f"Bearer {get_access_token}"},
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_station_location_history(
    ...
    response = client.put(
        f"/climsoft/v1/station-location-histories/{belongs_to}/{opening_datetime}",
...
def test_should_delete_station_location_history(
    client: TestClient, get_station_location_history, get_access_token: str
):
    station_location_history_data = (
        stationlocationhistory_schema.StationLocationHistory.from_orm(
            get_station_location_history
