...
def test_should_return_single_form_agro1(client: TestClient, get_form_agro1: climsoft_models.FormAgro1):
    response = client.get(
        f"/test/climsoft/v1/form_agro1s/{get_form_agro1.stationId}/{get_form_agro1.yyyy}/{get_form_agro1.mm}/{get_form_agro1.dd}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_form_agro1(client: TestClient, get_form_agro1):
    ...
    response = client.put(
        f"/test/climsoft/v1/form_agro1s/{station_id}/{yyyy}/{mm}/{dd}",
