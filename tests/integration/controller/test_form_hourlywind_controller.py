...
def test_should_return_single_form_hourlywind(client: TestClient, get_form_hourlywind: climsoft_models.FormHourlywind):
    response = client.get(
        f"/v1/form_hourlywinds/{get_form_hourlywind.stationId}/{get_form_hourlywind.yyyy}/{get_form_hourlywind.mm}/{get_form_hourlywind.dd}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_form_hourlywind(client: TestClient, get_form_hourlywind):
    ...
    response = client.put(
        f"/v1/form_hourlywinds/{station_id}/{yyyy}/{mm}/{dd}",
