...
def get_obs_element_by_id(element_id: str, db_session: Session = Depends(deps.get_session)):
    try:
...
def create_obs_element(data: obselement_schema.CreateObsElement, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def update_obs_element(element_id: str, data: obselement_schema.UpdateObsElement, db_session: Session = Depends(deps.get_session)):
    try:
        return get_success_response(
...
def delete_obs_element(element_id: str, db_session: Session = Depends(deps.get_session)):
    try:
