...
def test_should_return_first_five_station_location_histories(
    ...
    response = client.get(
        "/test/climsoft/v1/fault-resolutions",
        params={"limit": 5},
    )
...
def test_should_return_single_fault_resolution(
    ...
    response = client.get(
        f"/test/climsoft/v1/fault-resolutions/{get_fault_resolution.resolvedDatetime}/{get_fault_resolution.associatedWith}",
    )
    assert response.status_code == 200
    response_data = response.json()
...
def test_should_update_fault_resolution(
    ...
    response = client.put(
        f"/test/climsoft/v1/fault-resolutions/{resolved_datetime}/{associated_with}",
